...
registers = ["#0","x0","x1","x2","x3","x4","x5","x6","x7","w8","[x0]","[x1]","[x2]","[x3]","[x4]","[x5]","[x6]","[x7]","[x8]"]

# Lista de instruções
instructions = ["ldr","svc","adc","cmp","mov", "add","sub",
...
def generate_assembly_and_test3(instruction):
    ...
    # Executar o NASM e redirecionar saída para out.txt
    nasm_command = f"/bin/aarch64-linux-gnu-as  {asm_file} -o {bin_file2} >> {output_file} 2>&1"
    nasm_result = subprocess.run(nasm_command, shell=True)
    nasm_command = f"/bin/aarch64-linux-gnu-objcopy {bin_file2} -O binary  {bin_file}   >> {output_file} 2>&1"
    nasm_result = subprocess.run(nasm_command, shell=True)
...
def generate_assembly_and_test2(instruction, register):
    ...
    nasm_command = f"/bin/aarch64-linux-gnu-as {asm_file} -o {bin_file2} >> {output_file} 2>&1"
    nasm_result = subprocess.run(nasm_command, shell=True)
    nasm_command = f"/aarch64-linux-gnu-objcopy {bin_file2} -O binary  {bin_file}   >> {output_file} 2>&1"
    nasm_result = subprocess.run(nasm_command, shell=True)
 # Verificar se houve erro no NASM
    if nasm_result.returncode == 0:
        # Se compilou corretamente, gerar o hexdump do binário com xxd
